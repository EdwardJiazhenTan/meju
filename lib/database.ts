...
export const userQueries = {
  ...
  getUserStats: (
    ...
    publicDishes: number;
    sharedDishes: number;
    dishesSharedWithMe: number;
  } => {
    const db = getDatabase();

    const totalDishes = db
      .prepare("SELECT COUNT(*) as count FROM dishes WHERE owner_id = ?")
      .get(userId) as { count: number };
    const publicDishes = db
      .prepare(
        "SELECT COUNT(*) as count FROM dishes WHERE owner_id = ? AND visibility = 'public'",
      )
      .get(userId) as { count: number };
    const sharedDishes = db
      .prepare(
        "SELECT COUNT(*) as count FROM dishes WHERE owner_id = ? AND visibility = 'shared'",
      )
      .get(userId) as { count: number };
    const dishesSharedWithMe = db
      .prepare(
        "SELECT COUNT(*) as count FROM dish_shares WHERE shared_with = ?",
      )
      .get(userId) as { count: number };
...
export const dishQueries = {
  ...
  searchPublicDishes: (
    ...
    return stmt.all(searchPattern, searchPattern, limit, offset) as Array<
      Dish & { owner_name: string }
    >;
  },

  // Get public dishes by meal type
  getPublicDishesByMeal: (
    meal: string,
    limit = 20,
    offset = 0,
  ): Array<Dish & { owner_name: string }> => {
    ...
    const stmt = db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
    return stmt.all(meal, limit, offset) as Array<
      Dish & { owner_name: string }
    >;
  },

  // Get public dishes by tag
  getPublicDishesByTag: (
    tag: string,
    limit = 20,
    offset = 0,
  ): Array<Dish & { owner_name: string }> => {
  ...
  getDishShare: (
    ...
    | { share_id: number; can_reshare: boolean; shared_date: string }
    | undefined => {
    const db = getDatabase();
    const stmt = db.prepare(
      "SELECT * FROM dish_shares WHERE dish_id = ? AND shared_with = ?",
    );
    return stmt.get(dishId, userId) as
      | { share_id: number; can_reshare: boolean; shared_date: string }
      | undefined;
  },
  ...
  getDishShares: (
    ...
    `);
    return stmt.all(dishId) as Array<{
      share_id: number;
      email: string;
      display_name: string;
      username: string;
  ...
  getSharedWithUser: (
    ...
    limit = 20,
    offset = 0,
  ): Array<Dish & { owner_name: string; shared_date: string }> => {
    const db = getDatabase();
    const stmt = db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
    return stmt.all(userId, limit, offset) as Array<
      Dish & { owner_name: string; shared_date: string }
    >;
  },
  ...
  updateDish: (
    dishId: number,
    updateData: Partial<CreateDishData>,
  ): Database.RunResult => {
    const db = getDatabase();
    const fields = Object.keys(updateData)
      .map((key) => `${key} = ?`)
  ...
  getDishTags: (dishId: number): string[] => {
    const db = getDatabase();
    const stmt = db.prepare(
      "SELECT tag FROM dish_tags WHERE dish_id = ? ORDER BY tag",
    );
  ...
  removeTagFromDish: (dishId: number, tag: string): Database.RunResult => {
    const db = getDatabase();
    const stmt = db.prepare(
      "DELETE FROM dish_tags WHERE dish_id = ? AND tag = ?",
    );
    return stmt.run(dishId, tag);
  },
  ...
  getDishIngredients: (dishId: number): DishIngredient[] => {
    const db = getDatabase();
    const stmt = db.prepare(`
      SELECT i.ingredient_id, i.name, i.unit, di.quantity, i.calories_per_unit
  ...
  getDishIngredientById: (
    ...
    const db = getDatabase();
    const stmt = db.prepare(
      "SELECT quantity FROM dish_ingredients WHERE dish_id = ? AND ingredient_id = ?",
    );
...
export const mealPlanQueries = {
  ...
  userHasAccessToDish: async (
    ...
    const ownedDish = db
      .prepare("SELECT dish_id FROM dishes WHERE dish_id = ? AND owner_id = ?")
      .get(dishId, userId);
    ...
    const publicDish = db
      .prepare(
        "SELECT dish_id FROM dishes WHERE dish_id = ? AND visibility = 'public'",
      )
      .get(dishId);
    ...
    const sharedDish = db
      .prepare(
        "SELECT dish_id FROM dish_shares WHERE dish_id = ? AND shared_with = ?",
      )
      .get(dishId, userId);
  ...
  addDishToMealPlan: (
    ...
    `,
      ...
      .get(dailyPlan.daily_plan_id, mealType) as
      | { slot_id: number }
      | undefined;
  ...
  // Add customized dish to meal plan
  addCustomizedDishToMealPlan: (
    userId: number,
    dayOfWeek: number,
    mealType: string,
    dishId: number,
    ...
    const dailyPlan = db
      .prepare(
        "SELECT plan_id FROM daily_plans WHERE user_id = ? AND day_of_week = ?",
      )
      .get(userId, dayOfWeek) as { plan_id: number } | undefined;
    ...
    let mealSlot = db
      .prepare(
        "SELECT slot_id FROM meal_slots WHERE plan_id = ? AND meal_type = ?",
      )
      .get(dailyPlan.plan_id, mealType) as { slot_id: number } | undefined;

    if (!mealSlot) {
      const stmt = db.prepare(
        "INSERT INTO meal_slots (plan_id, meal_type) VALUES (?, ?)",
      );
      const result = stmt.run(dailyPlan.plan_id, mealType);
      mealSlot = { slot_id: result.lastInsertRowid as number };
    }

    // Add dish to meal slot with customizations
    const stmt = db.prepare(`
      INSERT OR REPLACE INTO meal_slot_dishes (slot_id, dish_id, serving_size, customizations)
      VALUES (?, ?, ?, ?)
    `);
    return stmt.run(mealSlot.slot_id, dishId, servingSize, customizations);
  },
};
...
export const ingredientQueries = {
  ...
  searchIngredients: (searchTerm: string): Ingredient[] => {
    const db = getDatabase();
    const stmt = db.prepare(
      "SELECT * FROM ingredients WHERE LOWER(name) LIKE LOWER(?) ORDER BY name",
    );
