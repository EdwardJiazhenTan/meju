...
export async function POST(
  ...
  try {
    ...
    const { dishId, mealType, servingSize = 1.0 } = body;
    ...
    const hasAccess = await mealPlanQueries.userHasAccessToDish(auth.user.userId, dishId);
    if (!hasAccess) {
      return NextResponse.json(
    ...
    mealPlanQueries.addDishToMealPlan(auth.user.userId, dayOfWeek, mealType, dishId, servingSize);
