...
export default function MealPlanPage() {
  ...
  const handleCustomizeDish = (
    dish: Dish,
    dayOfWeek: number,
    mealType: string,
  ) => {
    // Navigate to customize page with dish and meal context
    router.push(
      `/${locale}/customize-dish/${dish.dish_id}?day=${dayOfWeek}&meal=${mealType}`,
    );
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div
        className={`max-w-7xl mx-auto py-6 px-4 transition-all duration-300 ${selectedMeal ? "mr-96" : ""}`}
      >
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-foreground">
            {t("mealPlan.title")}
          </h1>
          <p className="text-foreground mt-1">{t("mealPlan.description")}</p>
        </div>
