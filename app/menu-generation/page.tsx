          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="bg-blue-100 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-blue-600">{result.summary.total_orders}</div>
                <div className="text-sm text-blue-800">处理订单数</div>
              </div>
              <div className="bg-green-100 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-green-600">{result.summary.total_meals_generated}</div>
                <div className="text-sm text-green-800">生成餐次数</div>
              </div>
              <div className="bg-purple-100 p-4 rounded-lg text-center">
                <div className="text-2xl font-bold text-purple-600">{result.summary.total_people_served}</div>
                <div className="text-sm text-purple-800">服务总人数</div>
              </div>
            </div>

            <div className="space-y-6">
              <h3 className="text-xl font-semibold text-gray-900">生成的菜单详情</h3>
                    ...
                    <div key={index} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="text-lg font-medium text-gray-900">
                          {new Date(mealPlan.date).toLocaleDateString('zh-CN')} - {getMealTypeText(mealPlan.meal_type)}
                        </h4>
                        <div className="text-sm text-gray-500">
                          {mealPlan.people_count}人 | {mealPlan.orders_count}个订单
                        </div>
                      </div>

                      <div className="space-y-2">
                        {mealPlan.dishes.map((dish: any, dishIndex: number) => (
                          <div key={dishIndex} className="flex justify-between items-center py-2 px-3 bg-gray-50 rounded">
                            <span className="font-medium">{dish.dish_name}</span>
                            <div className="text-sm text-gray-600">
                              {dish.people_count}人
                              {dish.notes && (
                                <span className="ml-2 text-gray-500">({dish.notes})</span>
                              )}
                            </div>
                          </div>
                        ))}
